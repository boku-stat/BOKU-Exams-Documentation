...
basic_template <- function(n = NULL, meta_name, meta_title, meta_version, types = NULL, ex_type, image = NULL) {
...
type_tabs <- tabsetPanel(
  ...
  tabPanel(
    "yes",
    radioButtons("type_for_all_questions", "Please select the type of question.", 
      choices = c("numeric", "text", "single choice", "multiple choice")
    )
  ), # if all questions have the same type 
  tabPanel("no", uiOutput("input_ui")) # different type for each question -> dynamic UI 
)
...
server <- function(input, output, session) {
  ...
  output$input_ui <- renderUI({
    req(input$question_choice == "yes") # Only render if questions are requested
    req(input$n) # Only proceed if number of questions is available
    lapply(1:input$n, function(i) {
      radioButtons(
        inputId = paste0("type_question", i),
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      if (input$question_choice == "yes") {
        if (!is.null(input$same_type) && input$same_type == "yes") {
          type <- switch(input$type_for_all_questions,
            "numeric" = "num",
            "text" = "string",
            "single choice" = "schoice",
      ...
      template <- basic_template(
        n = input$n, meta_name = input$meta_name,

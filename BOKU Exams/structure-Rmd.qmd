...
The `explanations` list **holds any information that can be possibly given as feedback**. This can, but does not have to, include the actual solutions, depending on the type of exam one is aiming for.
...
### Examplary structure of the Meta-information

Below is an example of how the "meta-information" section could look with some of the `R/exams` tags explained above, as well as automatic conversion to the correct format with `paste()`. 
...
????? remove backslashes for execution 
